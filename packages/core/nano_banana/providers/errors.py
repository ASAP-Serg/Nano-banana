...
_CLOUDFLARE_GATEWAY_MESSAGES = {
    "502": "Moonez временно недоступен (502): ошибка шлюза. Попробуйте через несколько минут.",
    "503": "Moonez временно недоступен (503): сервис перегружен. Попробуйте позже.",
    "521": "Moonez временно недоступен (521): сервер провайдера не отвечает. Попробуйте через несколько минут.",
    "522": "Moonez временно недоступен (522): не удалось подключиться к серверу провайдера.",
    "523": "Moonez временно недоступен (523): сервер провайдера недостижим.",
    "524": "Moonez временно недоступен (524): провайдер не ответил вовремя.",
}
...
def humanize_api_error(
    ...
    if _looks_like_html(text):
        ...
        if http_status and str(http_status) in _CLOUDFLARE_GATEWAY_MESSAGES:
            return _CLOUDFLARE_GATEWAY_MESSAGES[str(http_status)]
        return "Сервис Moonez вернул страницу ошибки вместо JSON. Попробуйте позже."
